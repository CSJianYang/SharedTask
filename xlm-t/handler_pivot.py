...
class Handler(BaseDynaHandler):
    ...
    def initialize(self, context):
        ...
        self.cfg = Namespace()
        self.fp16 = False
        self.cfg.generation = OmegaConf.create({'_name': None, 'beam': 1, 'nbest': 1, 'max_len_a': 0.0, 'max_len_b': 256, 'min_len': 1, 'unnormalized': False, 'lenpen': 1.0, 'unkpen': 0.0})
...
def handle_mini_batch(service, samples):
    ...
    if samples[-1]['sourceLanguage'] != "eng" and samples[-1]['targetLanguage'] != "eng" and "{}-{}".format(ISO2OUR[samples[0]['sourceLanguage']], ISO2OUR[samples[0]['targetLanguage']]) in PIVOT_TABLE:
        src2pivot_samples = [
            {
                'uid': samples[i]['uid'],
...
def handle(torchserve_data, context):
    ...
    for i, sample in enumerate(all_samples):
        if len(samples) > 0 and (samples[-1]['sourceLanguage'] != sample['sourceLanguage'] or samples[-1]['targetLanguage'] != sample['targetLanguage']):
            print("Processing BatchId: {} | Batchsize: {}".format(batch_id, len(samples)))
...
def local_test_from_file_small_track2():
    ...
    sourceLanguages = ['ind', 'eng', 'ind']
    targetLanguages = ['tam', 'ind', 'eng']
    output_file = "/home/v-jiaya/SharedTask/data/thunder/flores101_dataset/dynalab.translation"
    data = []
    for input_file, sourceLanguage, targetLanguage in zip(input_files, sourceLanguages, targetLanguages):
        data.extend(read_from_file(input_file, sourceLanguage, targetLanguage))
