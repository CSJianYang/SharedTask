...
MODEL=/mnt/input/SharedTask/thunder/small_task2/Filter_v1/model/adapter/24L-12L/
...
LANG_PAIRS="en-id,id-en,en-jv,jv-en,en-ms,ms-en,en-ta,ta-en,en-tl,tl-en,jv-id,id-ms,tl-id,jv-ms,ms-jv,ta-jv,jv-tl,tl-jv,ta-ms,tl-ms,ta-tl"
...
BT_LANG_APIRS="en-id,en-jv,en-ms,en-ta,en-tl,jv-id,id-ms,tl-id,jv-ms,ms-jv,ta-jv,jv-tl,tl-jv,ta-ms,tl-ms,ta-tl"
...
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=8 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --langs $LANGS --reset-optimizer --reset-lr-scheduler --reset-dataloader --reset-meters --data-param-list-sampling-ratios "{\"main\":0.6,\"bt\":0.3,\"parallel_bt\":0.1}" \
    ...
    --optimizer adam --adam-betas '(0.9, 0.98)' --lr-scheduler inverse_sqrt --lr 1e-5 --warmup-init-lr 1e-07 --stop-min-lr 1e-09 --warmup-updates 4000 \
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 3072 --update-freq 32 \
    --adapter-dim 256 --start-adapter-layer 10 --enable-lang-ids --same-lang-per-batch --freeze-encoder --freeze-decoder --freeze-embedding \
