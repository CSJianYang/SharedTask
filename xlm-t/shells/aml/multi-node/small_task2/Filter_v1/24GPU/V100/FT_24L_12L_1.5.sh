...
#Parallel BT
PARALLEL_BT_TEXT=/mnt/input/SharedTask/thunder/small_task2/Filter_v1/parallel_bt_split1.5-data-bin10/
...
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=3 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --langs $LANGS --data-param-list-sampling-ratios "{\"main\":0.5,\"bt\":0.25,\"parallel_bt\":0.25}" \
