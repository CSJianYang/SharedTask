...
MODEL=/mnt/input/SharedTask/large-scale/small_task1/download/model/M2M100-lr1e-4-maxpos256/
...
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=2 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --share-all-embeddings --max-source-positions 256 --max-target-positions 256 --criterion label_smoothed_cross_entropy_with_sparse --label-smoothing 0.1 \
    ...
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 2048 --update-freq 16 --enable-reservsed-directions-shared-datasets --virtual-epoch-size 100000000 \
    --seed 1 --log-format simple --skip-invalid-size-inputs-valid-test --fp16 \
    --ddp-backend=no_c10d 2>&1 | tee -a $MODEL/train.log

#--reset-meters --reset-dataloader --reset-optimizer --reset-lr-scheduler 
