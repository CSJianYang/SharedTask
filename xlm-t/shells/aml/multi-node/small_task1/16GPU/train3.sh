...
MODEL=/mnt/input/SharedTask/large-scale/small_task1/download/model/M2M100-lr2e-4/
...
echo "Start Training ..."
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=2 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --optimizer adam --adam-betas '(0.9, 0.98)' --lr-scheduler inverse_sqrt --lr 2e-4 --warmup-init-lr 1e-07 --stop-min-lr 1e-09 --warmup-updates 4000 \
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 1536 --update-freq 20 --enable-reservsed-directions-shared-datasets \
    --seed 1 --log-format simple --skip-invalid-size-inputs-valid-test --fp16 --same-lang-per-batch --enable-lang-ids --virtual-epoch-size 100000000 \
