#BT
BT_TEXT=/mnt/input/SharedTask/thunder/large_track/data/bt_split-data-bin80/
...
#PARALLEL BT
PARALLEL_BT_TEXT=/mnt/input/SharedTask/thunder/large_track/data/parallel_bt_split-data-bin80/
...
#Bitext
TEXT=/mnt/input/SharedTask/thunder/large_track/data/split-data-bin80/
...
#LANG PAIRS
LANG_PAIRS=./shells/aml/multi-node/large_task1/deltalm/lang_pairs.txt
BT_LANG_PAIRS=./shells/aml/multi-node/large_task1/deltalm/bt_lang_pairs.txt
PARALLEL_BT_LANG_PAIRS=./shells/aml/multi-node/large_task1/deltalm/parallel_bt_language_pairs.txt

#Pretrained Model
PRETRAINED_MODEL_DIR=/mnt/input/SharedTask/thunder/PretrainedModel/deltalm/large-postnorm/
PRETRAINED_MODEL=$PRETRAINED_MODEL_DIR/model_state_update_300000.th

#Save Dir
MODEL=/mnt/input/SharedTask/thunder/large_track/data/model/deltalm/FULL-v1/lr1e-4-deltalm-postnorm-96GPU/
mkdir -p $MODEL
echo "Start Training ..."
echo "$TEXT"
echo "$BT_TEXT"
echo "$PARALLEL_BT_TEXT"
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=12 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $DATA_BIN \
    ...
    --optimizer adam --adam-betas '(0.9, 0.98)' --lr-scheduler inverse_sqrt --lr 1e-4 --warmup-init-lr 1e-07 --stop-min-lr 1e-09 --warmup-updates 4000 --enable-reservsed-directions-shared-datasets \
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 1536 --update-freq 8 \
