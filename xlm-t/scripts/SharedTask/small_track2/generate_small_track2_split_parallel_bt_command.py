...
def parse_args():
    ...
    parser.add_argument('--parallel-bt-lang-pairs', '-parallel-bt-lang-pairs', type=str,
                        default=r'/mnt/input/SharedTask/thunder/small_task2/Filter_v1/parallel_bt-train/', help='input stream')
...
if __name__ == "__main__":
    ...
    cmds = ""
    input_dir = "/mnt/input/SharedTask/thunder/small_task2/Filter_v1/parallel_bt-train/"
    split_num = 10
    output_dir = "/mnt/input/SharedTask/thunder/small_task2/Filter_v1/parallel_bt-split{}/".format(split_num)
    for lang in parallel_bt_lang_pairs:
        input = os.path.join(input_dir, "train.{}".format(lang))
        output = output_dir
        cmds += "- name: small_task2_parallel_bt_split_v1_{}\n  sku: G0\n  sku_count: 1\n  command: \n    - python ./scripts/SharedTask/SplitTrainingData.py -input {} -output {} -split-num {}\n".format(lang, input, output, split_num)
    print(cmds)