...
def parse_args():
    ...
    parser.add_argument('--checkpoint-name', '-checkpoint-name', type=str,
                        default=r'/mnt/input/SharedTask/thunder/small_task2/Filter_v1/model/36L-12L-average/avg.pt', help='input stream')
            ...
            if src != tgt:
                try:
                    with open(os.path.join(args.log, "{}-{}.BLEU".format(src, tgt)), "r", encoding="utf-8") as r:
                        ...
                        for i in range(len(result_lines) - 1, 0, -1): #reversed search
                            if checkpoint_name == result_lines[i].strip().replace("MODEL: ", ""):
