...
if __name__ == "__main__":
    ...
    cmds = ""
    input_dir = "/mnt/input/SharedTask/thunder/large_track/data/Filter_v1/bt_spm/"
    output_dir = "/mnt/input/SharedTask/thunder/large_track/data/Filter_v1/bt_split80/"
    for lang_pair in bt_lang_pairs:
        src, tgt = lang_pair.split('-')
        input = os.path.join(input_dir, "{}{}".format(src, tgt))
        output = output_dir
        cmds += "- name: bt_split_v1_{}-{}\n  sku: G0\n  sku_count: 1\n  command: \n    - python ./scripts/SharedTask/SplitTrainingData.py -input {} -output {} \n".format(src, tgt, input, output)
    print(cmds)