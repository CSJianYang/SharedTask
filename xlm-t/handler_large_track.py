...
class Handler(BaseDynaHandler):
    ...
    def initialize(self, context):
        ...
        self.cfg = Namespace()
        self.fp16 = False
        self.cfg.generation = OmegaConf.create({'_name': None, 'beam': 1, 'nbest': 1, 'max_len_a': 0.0, 'max_len_b': 64, 'min_len': 1, 'unnormalized': False, 'lenpen': 0.8, 'unkpen': 0.0})
        ...
        self.truncate_source = min(100, self.max_positions[0] - 3)
        self.taskIO = TaskIO()
...
def local_test():
    from dynalab.tasks import flores_full

    bin_data = b"\n".join(json.dumps(d).encode("utf-8") for d in flores_full.data)
    torchserve_data = [{"body": bin_data}]

    #manifest = {"model": {"serializedFile": "model.pt"}}
    manifest = {"model": {"serializedFile": "large_track_24L_6L.pt"}}
