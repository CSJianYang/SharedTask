...
def parse_args():
    ...
    parser.add_argument('--checkpoint-name', '-checkpoint-name', type=str,
                        default=r'/mnt/input/SharedTask/thunder/large_track/data/Filter_v1/model/24L-12L/avg61_80.pt', help='input stream')
    parser.add_argument('--result', '-result', type=str,
                        default=r'/mnt/input/SharedTask/ExcelResults/', help='input stream')
            ...
            if src != tgt:
                try:
                    with open(os.path.join(args.log, "{}-{}.BLEU".format(src, tgt)), "r", encoding="utf-8") as r:
                        ...
                        print("Can not find {}".format(os.path.join(args.log, "{}-{}.BLEU".format(src, tgt))))
