...
def get_pivot_table(path):
    ...
    if len(pivot_lines) > 0 and pivot_lines[0].strip() != "":
        ...
        pivot_table = []
    return pivot_table
PIVOT_TABLE = get_pivot_table("small_track1_pivot_pairs.txt")
...
def handle(torchserve_data, context):
    ...
    print("DIRECT SAMPLES: {}".format(direct_n))
    print("PIVOT SAMPLES: {}".format(pivot_n))
    logger.info(
...
def local_test_small_track1():
    ...
    manifest = {"model": {"serializedFile": "small_track1_36L_12L.pt"}}
...
def local_test_small_track2():
    from dynalab.tasks import flores_small1, flores_small2

    bin_data = b"\n".join(json.dumps(d).encode("utf-8") for d in flores_small2.data)
    torchserve_data = [{"body": bin_data}]

    manifest = {"model": {"serializedFile": "small_track1.pt"}}
    system_properties = {"model_dir": ".", "gpu_id": 0}
