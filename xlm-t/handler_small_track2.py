...
class Handler(BaseDynaHandler):
    ...
    def initialize(self, context):
        ...
        self.cfg = Namespace()
        self.fp16 = False
        self.cfg.generation = OmegaConf.create({'_name': None, 'beam': 4, 'nbest': 1, 'max_len_a': 0.0, 'max_len_b': 196, 'min_len': 1, 'unnormalized': False, 'lenpen': 1.0, 'unkpen': 0.0})
        ...
        if MODEL_TYPE == "Our":
            ...
            self.src_dict = Dictionary.load("m2m_dict.txt")
        ...
        print("Model: {} | Parameters: {}".format(model_pt_path, sum(p.numel() for p in self.models[0].parameters())))
        # Set model
...
def local_test():
    ...
    if MODEL_TYPE == "M2M":
        manifest = {"model": {"serializedFile": "m2m_615M.pt"}} #small_track2_24L_12L.pt / m2m_615M.pt
    else:
        manifest = {"model": {"serializedFile": "small_track2_36L_12L.pt"}}  # small_track2_24L_12L.pt / m2m_615M.pt
