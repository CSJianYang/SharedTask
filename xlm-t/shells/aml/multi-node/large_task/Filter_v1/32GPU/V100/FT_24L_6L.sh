#BT
BT_TEXT=/mnt/input/SharedTask/thunder/large_track/data/Filter_v1/bt_split-data-bin80/
...
#PARALLEL BT
PARALLEL_BT_TEXT=/mnt/input/SharedTask/thunder/large_track/data/Filter_v1/parallel_bt_split-data-bin80/
...
#Bitext
TEXT=/mnt/input/SharedTask/thunder/large_track/data/Filter_v1/split-data-bin80/
...
#Save Dir
MODEL=/mnt/input/SharedTask/thunder/large_track/data/model/deltalm/FULL-v1/lr1e-4-deltalm-postnorm-24L-6L/
mkdir -p $MODEL
echo "Start Training ..."
echo "$TEXT"
echo "$BT_TEXT"
...
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=4 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $DATA_BIN \
    ...
    --optimizer adam --adam-betas '(0.9, 0.98)' --lr-scheduler inverse_sqrt --lr 1e-4 --warmup-init-lr 1e-07 --stop-min-lr 1e-09 --warmup-updates 4000 --enable-reservsed-directions-shared-datasets --virtual-epoch-size 300000000 --data-param-list-sampling-ratios '{"main":0.5,"bt":0.25,"parallel_bt":0.25}' \
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 2048 --update-freq 24 --decoder-layers 6 --clip-norm 0.25 \
