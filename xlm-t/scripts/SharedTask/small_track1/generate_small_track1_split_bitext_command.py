...
if __name__ == "__main__":
    ...
    cmds = ""
    split_num = 10
    input_dir = "/mnt/input/SharedTask/thunder/small_task1/Filter_v1/train/"
    output_dir = "/mnt/input/SharedTask/thunder/small_task1/Filter_v1/split{}/".format(split_num)
    for lang_pair in lang_pairs:
        src , tgt = lang_pair.split('-')
        output = output_dir
        input = os.path.join(input_dir, "train.{}-{}.{}".format(src, tgt, src))
        cmds += "- name: split_v1_{}-{}.{}\n  sku: G0\n  sku_count: 1\n  command: \n    - python ./scripts/SharedTask/SplitTrainingData.py -input {} -output {} -split-num {}\n".format(src, tgt, src, input, output, split_num)
        input = os.path.join(input_dir, "train.{}-{}.{}".format(src, tgt, tgt))
        cmds += "- name: split_v1_{}-{}.{}\n  sku: G0\n  sku_count: 1\n  command: \n    - python ./scripts/SharedTask/SplitTrainingData.py -input {} -output {} -split-num {}\n".format(src, tgt, tgt, input, output, split_num)
    print(cmds)