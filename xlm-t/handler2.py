...
class Handler(BaseDynaHandler):
    ...
    def preprocess(self, examples):
        ...
        print(examples)
        src_tokens = [self.task.source_dictionary.encode_line(encode_fn(example["sourceText"], self.bpe, self.tokenizer), add_if_not_exist=False).long()[:self.max_positions[0] - 1] for example in examples]
        src_tokens = data_utils.collate_tokens(
            src_tokens,
            self.task.source_dictionary.pad_index,
            self.task.source_dictionary.eos_index,
            left_pad=True,
            move_eos_to_beginning=False,
            pad_to_length=None,
            pad_to_multiple=1,
        )
        src_lang_toks = torch.cat([torch.LongTensor([self.task.data_manager.get_encoder_langtok(LANG_CODE[example["sourceLanguage"]], LANG_CODE[example["targetLanguage"]], spec=self.src_langtok_spec)]) for example in examples]).unsqueeze(1)
        src_tokens = torch.cat([src_lang_toks, src_tokens], dim=1)
        print(src_tokens.size())
        src_lengths = torch.LongTensor([src_tokens[i].ne(self.task.source_dictionary.pad_index).long().sum() for i in range(src_tokens.size(0))])
        tgt_prefix_tokens = torch.cat([torch.LongTensor([self.task.data_manager.get_decoder_langtok(LANG_CODE[example["targetLanguage"]], spec=self.tgt_langtok_spec)], dim=0) for example in examples]).unsqueeze(1)
        print(src_tokens)
        print(src_lengths)
        print(src_lang_toks)
        print(tgt_prefix_tokens.size())
        input_data = {
            "net_input": {
            ...
            "examples": examples,
            "tgt_prefix_tokens": tgt_prefix_tokens
        }
        return input_data
    ...
    def inference(self, input_data):
        ...
        hypos = self.task.inference_step(
            self.generator, self.models, input_data, prefix_tokens=input_data["tgt_prefix_tokens"]
        )
        # #################################################
        for i,hypo in enumerate(hypos[: min(len(hypos), self.cfg.generation.nbest)]):
            #print(hypo[0]['tokens'])
            hypo_tokens, hypo_str, alignment = utils.post_process_prediction(
                hypo_tokens=hypo[i]["tokens"].int().cpu(),
                src_str=input_data["examples"][i]["src_str"],
                alignment=hypo[i]["alignment"],
                align_dict=None,
...
def handle_mini_batch(service, samples):
    ...
    response = service.postprocess(output, samples)
    return response
...
def local_test():
    ...
    manifest = {"model": {"serializedFile": "simple_model.pt"}}
