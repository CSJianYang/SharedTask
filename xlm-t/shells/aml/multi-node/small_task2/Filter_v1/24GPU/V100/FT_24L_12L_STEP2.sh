...
MODEL=/mnt/input/SharedTask/thunder/small_task2/Filter_v1/model/FT/
...
echo "Start Training ..."
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=3 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --lang-pairs $LANG_PAIRS --truncate-source --enable-reservsed-directions-shared-datasets --virtual-epoch-size 2500000000 \
    ...
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 1536 --update-freq 128 \
