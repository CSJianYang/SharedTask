...
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=4 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 4096 --update-freq 32 \
    --adapter-dim 256 --start-adapter-layer 6 --enable-lang-ids --same-lang-per-batch --freeze-encoder --freeze-decoder --freeze-embedding \
