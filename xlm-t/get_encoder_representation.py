...
def run(args):
    ...
    model = load_mt_model(args.ckpt_path)
    ...
    with open("{}.{}".format(args.src_fn, args.suffix), "w") as w:
        with torch.no_grad():
            for sample in dl:
                encoder_out = model.encoder(sample['net_input']['src_tokens'], sample['net_input']['src_lengths'], return_all_hiddens=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt_path', default="/home/v-jiaya/XMT/premodels/x2e/avg4_8.pt", type=str)
    # parser.add_argument('--roberta_model_path', default="", type=str)
    parser.add_argument("--src_fn", type=str,
                        default="/home/v-jiaya/XMT/data/analysis/similarity/parallel.en")
    parser.add_argument('--vocab_path', default="/home/v-jiaya/SharedTask/xlm-t/dict.txt", type=str)
