...
MODEL=/mnt/input/SharedTask/large-scale/small_task1/download/model/M2M100-lr1e-4/
...
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=4 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 3072 --update-freq 8 --enable-reservsed-directions-shared-datasets \
