...
def parse_args():
    ...
    parser.add_argument('--bitext', '-bitext', type=str,
                        default=r'./results/large_track_bitext_lines.txt', help='input stream')
    parser.add_argument('--bt', '-bt', type=str,
                        default=r'./results/large_track_bt_lines.txt', help='input stream')
    parser.add_argument('--parallel-bt', '-parallel-bt', type=str,
                        default=r'./results/large_track_parallel_bt_lines.txt', help='input stream')
    parser.add_argument('--output', '-output', type=str,
                        default=r'./results/large_track.txt', help='input stream')
...
if __name__ == "__main__":
    ...
    print(bt)
    print(parallel_bt)
    print("BITEXT: {} | {} pairs".format(sum(bitext.values()), len(bitext)))
    print("BT: {} | {} pairs".format(sum(bt.values()), len(bt)))
    print("PARALLEL BT: {} | {} langs".format(list(parallel_bt.values())[0], len(parallel_bt)))
    ...
    with open(args.output, "w", encoding="utf-8") as w:
        ...
        w.write("{}\n".format(bt))
        w.write("{}\n".format(parallel_bt))
        print("Successfully saving to {}".format(args.output))
