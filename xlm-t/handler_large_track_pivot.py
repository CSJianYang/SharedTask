...
def get_pivot_table(path):
    ...
    if len(pivot_lines) > 0 and pivot_lines[0].strip() != "":
        ...
        pivot_table = []
    return pivot_table
PIVOT_TABLE = get_pivot_table("large_track_pivot_pairs.txt")
...
class Handler(BaseDynaHandler):
    ...
    def initialize(self, context):
        ...
        self.cfg = Namespace()
        self.fp16 = False
        self.cfg.generation = OmegaConf.create({'_name': None, 'beam': 2, 'nbest': 1, 'max_len_a': 0.0, 'max_len_b': 160, 'min_len': 1, 'unnormalized': False, 'lenpen': 1.0, 'unkpen': 0.0})
        ...
        self.truncate_source = min(160, self.max_positions[0] - 3)
        self.taskIO = TaskIO()
...
def local_test():
    ...
    manifest = {"model": {"serializedFile": "large_track_24L_12L.pt"}}
