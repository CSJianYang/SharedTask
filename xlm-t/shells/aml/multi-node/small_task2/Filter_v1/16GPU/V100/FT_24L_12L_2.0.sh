...
#Parallel BT
PARALLEL_BT_TEXT=/mnt/input/SharedTask/thunder/small_task2/Filter_v1/parallel_bt_split2.0-data-bin10/
...
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=2 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --langs $LANGS --reset-optimizer --reset-lr-scheduler --reset-dataloader --reset-meters \
    ...
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 1792 --update-freq 32 \
