...
MODEL=/mnt/input/SharedTask/thunder/small_task2/Filter_v1/model/24L-12L-A100/
...
echo "Start Training ..."
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=6 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --langs $LANGS --data-param-list-sampling-ratios "{\"main\":0.7,\"bt\":0.1,\"parallel_bt\":0.2}" \
    ...
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 2304 --update-freq 32 \
