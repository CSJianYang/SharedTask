...
MODEL=/mnt/input/SharedTask/thunder/small_task2/download/model/deltalm/lr1e-4-deltalm-postnorm/
...
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=4 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $TEXT \
    ...
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 1536 --update-freq 16 --virtual-epoch-size 200000000 \
