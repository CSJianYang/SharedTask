...
def get_pivot_table(path):
    ...
    if len(pivot_lines) > 0 and pivot_lines[0].strip() != "":
        pivot_table = pivot_lines[0].strip().split(",")
    else:
        pivot_table = []
    return pivot_table
...
class Handler(BaseDynaHandler):
    ...
    def initialize(self, context):
        ...
        self.cfg = Namespace()
        self.fp16 = False
        self.cfg.generation = OmegaConf.create({'_name': None, 'beam': 4, 'nbest': 1, 'max_len_a': 0.0, 'max_len_b': 155, 'min_len': 1, 'unnormalized': False, 'lenpen': 1.0, 'unkpen': 0.0})
        ...
        self.truncate_source = min(155, self.max_positions[0] - 3)
        self.taskIO = TaskIO()
    ...
    def preprocess_one(self, sample) -> dict:
        ...
        if MODEL_TYPE == "M2M":
            ...
            return {
                ...
                "src_length": len(tokens) + 1,
                "src_lang_id": list(ISO2SIMP.keys()).index(sample["sourceLanguage"]) + 1,
                "tgt_lang_id": list(ISO2SIMP.keys()).index(sample["targetLanguage"]) + 1,
            }
    ...
    def preprocess(self, samples) -> dict:
        ...
        if MODEL_TYPE == "Our":
            src_lang_id = [sample['src_lang_id'] for sample in samples]
            tgt_lang_id = [sample['tgt_lang_id'] for sample in samples]
            return {
...
def handle(torchserve_data, context):
    ...
    n = len(all_samples)
    all_samples = sorted(all_samples, key=lambda x: x['targetLanguage']) # sort by target language id

    direct_all_samples = list(filter(lambda x: _lang_pair(x['sourceLanguage'], x['targetLanguage']) not in PIVOT_TABLE, all_samples))
    pivot_all_samples = list(filter(lambda x: _lang_pair(x['sourceLanguage'], x['targetLanguage']) in PIVOT_TABLE, all_samples))
    ...
    for i, direct_sample in enumerate(direct_all_samples):
        ...
        langs = list(set([sample["targetLanguage"] for sample in direct_samples]))
        if len(langs) > 1:
            direct_samples_lang1 = list(filter(lambda x: x["targetLanguage"] == langs[0], direct_samples))
            translations_lang1 = handle_mini_batch(_service, direct_samples_lang1)
            direct_samples_lang2 = list(filter(lambda x: x["targetLanguage"] == langs[1], direct_samples))
            translations_lang2 = handle_mini_batch(_service, direct_samples_lang2)
            translations = translations_lang1 + translations_lang2
        else:
    ...
    for i, pivot_sample in enumerate(pivot_all_samples):
        ...
        langs = list(set([sample["targetLanguage"] for sample in direct_samples]))
        if len(langs) > 1:
            pivot_samples_lang1 = list(filter(lambda x: x["targetLanguage"] == langs[0], pivot_samples))
            translations_lang1 = handle_mini_batch(_service, pivot_samples_lang1)
            pivot_samples_lang2 = list(filter(lambda x: x["targetLanguage"] == langs[1], pivot_samples))
            translations_lang2 = handle_mini_batch(_service, pivot_samples_lang2)
            translations = translations_lang1 + translations_lang2
        else:
...
def local_test():
    ...
    manifest = {"model": {"serializedFile": "small_track2_adapter.pt"}}
...
def local_test_from_file_from_small_track1():
    ##### Read from the Files ################
    input_files = ["/home/v-jiaya/SharedTask/data/thunder/flores101_dataset/devtest-code/valid.hr", "/home/v-jiaya/SharedTask/data/thunder/flores101_dataset/devtest-code/valid.sr"]
...
def local_test_from_file_small_track2():
    ##### Read from the Files ################
    input_files = ["/home/v-jiaya/SharedTask/data/thunder/flores101_dataset/devtest-code/valid.id", "/home/v-jiaya/SharedTask/data/thunder/flores101_dataset/devtest-code/valid.en", "/home/v-jiaya/SharedTask/data/thunder/flores101_dataset/devtest-code/valid.id"]
