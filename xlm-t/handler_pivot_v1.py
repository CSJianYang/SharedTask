...
def mapping(languages: str) -> dict:
    return dict(
        tuple(pair.split(":"))
        for pair in languages.strip().replace(" ", "").replace("\n", "").split(",")
    )
...
class Handler(BaseDynaHandler):
    ...
    def initialize(self, context):
        ...
        self.cfg = Namespace()
        self.fp16 = False
        self.cfg.generation = OmegaConf.create({'_name': None, 'beam': 3, 'nbest': 1, 'max_len_a': 0.0, 'max_len_b': 200, 'min_len': 1, 'unnormalized': False, 'lenpen': 1.0, 'unkpen': 0.0})
...
def is_pivot(lang1, lang2):
    pair_str = ISO2OUR[lang1] + '-' + ISO2OUR[lang2]
    if pair_str not in PIVOT_TABLE:
        return False
    if lang1 == 'eng' or lang2 == 'eng':  # X->E or E->X
        return False
    if (lang1 == 'zho_simp' and lang2 == 'zho_trad') or (lang1 == 'zho_trad' and lang2 == 'zho_simp'):
        return False
    if (lang1 == 'zho_simp' and lang2 == 'jpn') or (lang1 == 'jpn' and lang2 == 'zho_simp'):
        return False
    if (lang1 == 'zho_simp' and lang2 == 'jpn') or (lang1 == 'jpn' and lang2 == 'zho_simp'):
        return False
    if (lang1 == 'zho_simp' and lang2 == 'kor') or (lang1 == 'kor' and lang2 == 'zho_simp'):
        return False
    if (lang1 == 'zho_simp' and lang2 == 'rus') or (lang1 == 'rus' and lang2 == 'zho_simp'):
        return False
    if (lang1 == 'zho_simp' and lang2 == 'tha') or (lang1 == 'tha' and lang2 == 'zho_simp'):
        return False
    if (lang1 == 'zho_simp' and lang2 == 'vie') or (lang1 == 'vie' and lang2 == 'zho_simp'):
        return False
...
def handle(torchserve_data, context):
    ...
    for i, sample in enumerate(all_samples):
        _src_lg = sample['sourceLanguage']
        _tgt_lg = sample['targetLanguage']

        if is_pivot(_src_lg, _tgt_lg):
            samples.append(sample)
        else:
            pivot_tgt_lgs.append(sample['targetLanguage'])
            pivot_sample = {
                'uid': sample['uid'],
        ...
        if len(samples) + len(pivot_samples) < batch_size and i + 1 < n:
            continue
        if len(samples) > 0:
            results.extend(handle_mini_batch(_service, samples))
        if len(pivot_samples) > 0:
            r_samples = []
            tmp_ret = handle_mini_batch(_service, pivot_samples)
            for si, p_sample in enumerate(tmp_ret):
                r_sample = {
                    'uid': p_sample['id'],
                    'sourceLanguage': 'eng',
                    'targetLanguage': pivot_tgt_lgs[si],
                    'sourceText': p_sample['translatedText']
                }
                r_samples.append(r_sample)
            results.extend(handle_mini_batch(_service, r_samples))
