...
def parse_args():
    ...
    parser.add_argument('--input-dir', '-input-dir', type=str,
                        default=r'/home/v-jiaya/SharedTask/data/OPUS/', help='input stream')
    parser.add_argument('--output-dir', '-output-dir', type=str,
                        default=r'/home/v-jiaya/SharedTask/data/OPUS/train/', help='input stream')
...
if __name__ == "__main__":
    ...
    for i, file in enumerate(files):
        if "train" in file and os.path.getsize(os.path.join(args.input_dir, file)) > 0:
            src, tgt = file.split('.')[-1].split('-')
            with open(os.path.join(args.input_dir, "train.{}-{}".format(src, tgt)), "r", encoding="utf-8") as r:
                src_lines = []
                tgt_lines = []
                lines = r.readlines()
                results = []
                ...
                for i, line in enumerate(lines):
                    ...
                    if line.strip() == "</tu>":
                        ...
                        if len(context) == 2:
                            ...
                            if src_line != "" and tgt_line != "" and len(src_line[0].split()) / len(tgt_line[0].split()) > 2.0 or len(src_line[0].split()) / len(tgt_line[0].split()) < 2.0:
                ...
                if len(src_lines) > 0 and len(tgt_lines) > 0:
                    ...
                    with open(os.path.join(args.output_dir, "{}{}".format(src, tgt), "train.{}-{}.{}".format(src, tgt, src)), "w", encoding="utf-8") as w_src:
                        with open(os.path.join(args.output_dir, "{}{}".format(src, tgt), "train.{}-{}.{}".format(src, tgt, tgt)), "w", encoding="utf-8") as w_tgt:
                            w_src.write("\n".join(src_lines))
                            w_tgt.write("\n".join(tgt_lines))
                            print("Successfully saving to {}".format(os.path.join(args.output_dir, "{}{}".format(src, tgt), "train.{}-{}.{}".format(src, tgt, src))))
                            print("Successfully saving to {}".format(os.path.join(args.output_dir, "{}{}".format(src, tgt), "train.{}-{}.{}".format(src, tgt, tgt))))
