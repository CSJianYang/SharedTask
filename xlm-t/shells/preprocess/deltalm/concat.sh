...
if [ "$op" == "small-task1-train" ]; then
    ...
    TEXT=/mnt/input/SharedTask/thunder/small_task1/download/small_task1_filt_spm/
    TRAIN=/mnt/input/SharedTask/thunder/small_task1/download/train/
    rm $TEXT/DGT.hr-hu.hr
    rm $TEXT/DGT.hr-hu.hu
    echo "Removing $TEXT/DGT.hr-hu.hr and $TEXT/DGT.hr-hu.hu (source lines != target lines)"
    mkdir -p $TRAIN
    for src in ${LANGS[@]}; do
        for tgt in ${LANGS[@]}; do
            echo "${src}->${tgt}"
