...
if __name__ == "__main__":
    ...
    cmds = ""
    input_dir = "/mnt/input/SharedTask/thunder/large_track/data/parallel_bt_spm/"
    output_dir = "/mnt/input/SharedTask/thunder/large_track/data/Filter_v1/parallel_bt_spm/"
    for lang_pair in lang_pairs:
        src , tgt = lang_pair.split('-')
        input_src = os.path.join(input_dir, "train.{}-{}.{}".format(src, tgt, src))
        input_tgt = os.path.join(input_dir, "train.{}-{}.{}".format(src, tgt, tgt))
        output_src = os.path.join(output_dir, "train.{}-{}.{}".format(src, tgt, src))
        output_tgt = os.path.join(output_dir, "train.{}-{}.{}".format(src, tgt, tgt))
        cmds += "- name: parallel_bt_filter_v1_{}-{}\n  sku: G0\n  sku_count: 1\n  command: \n    - python ./scripts/SharedTask/large_track/Filter_v1/filter_parallel_bt.py -src {} -tgt {} -new-src {} -new-tgt {} \n".format(src, tgt, input_src, input_tgt, output_src, output_tgt)
    print(cmds)