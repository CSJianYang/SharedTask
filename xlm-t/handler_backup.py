...
class Handler(BaseDynaHandler):
    ...
    def initialize(self, context):
        ...
        self.device = device
        ### Initialization ###
        self.model_type = "M2M"  # M2M and Our
        self.cfg = Namespace()
    ...
    def lang_token(self, lang: str) -> int:
        ...
        if self.model_type == "M2M":
            ...
            simple_lang = ISO2OUR[lang]
        token = self.src_dict.indices[f"__{simple_lang}__"]
        return token


    def tokenize(self, line: str) -> list:
        tokens = self.src_dict.encode_line(encode_fn(line, self.bpe, self.tokenizer), add_if_not_exist=False).long()[:self.max_positions[0] - 3]
