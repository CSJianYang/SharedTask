...
#Parallel BT
PARALLEL_BT_TEXT=/mnt/input/SharedTask/thunder/small_task1/Filter_v1/parallel_bt_split-data-bin10/
...
MODEL=/mnt/input/SharedTask/thunder/small_task1/Filter_v1/model/36L-12L-FT/
...
echo "Start Training ..."
python -m torch.distributed.launch --nproc_per_node=8 --nnodes=2 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_ADDR" --master_port=$MASTER_PORT train.py $DATA_BIN \
    ...
    --langs $LANGS \
    --lang-pairs $LANG_PAIRS --truncate-source --enable-reservsed-directions-shared-datasets --data-param-list-sampling-ratios "{\"main\":0.6,\"bt\":0.2,\"parallel_bt\":0.2}" \
    ...
    --max-update 400000 --max-epoch 100 --weight-decay 0.0 --max-tokens 1792 --encoder-layers 36 --update-freq 64 --clip-norm 1.0 \
